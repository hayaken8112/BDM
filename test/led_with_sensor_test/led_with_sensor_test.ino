...
//------------------------------------------------------------------------
Adafruit_NeoPixel strip = Adafruit_NeoPixel(30, 4, NEO_GRB + NEO_KHZ800); //LEDtape
...
// LED Initialize all pixels to 'off'
  strip.begin();
  strip.show(); 
...
void printSensorData(){
  ...
  Attitude(imu.ax, imu.ay, imu.az, -imu.my, -imu.mx, imu.mz);
  Serial.println();
  delay(PRINT_SPEED);
}
...
void Attitude(float ax, float ay, float az, float mx, float my, float mz)
{
  ...
  printAttitude(pitch, roll, heading);
  controlLEDbyAttitude(pitch,roll,heading);
  
}

void printAttitude(float pitch, float roll,float heading){
...
//LED
void controlLEDbyAttitude(float pitch, float roll, float heading){
  if (40 < heading && heading < 50){
    colorWipe(strip.Color(20,0,0),5);//Red
    delay(strip.numPixels() * 5);
  }
}
